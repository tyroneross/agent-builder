...
function buildGovernanceManifest(spec, specProfile) {
  const sideEffects = [...new Set((spec.tools ?? []).map((tool) => tool.sideEffect ?? "unspecified").filter(Boolean))];
  const toolTiers = (spec.tools ?? []).map((tool) => ({
    tool: tool.name,
...
function buildManifest(spec, createdAt, specProfile) {
  ...
  return {
    ...
    evals: spec.evals,
    governance: buildGovernanceManifest(spec, specProfile),
...
function buildSystemBoundaryContract(spec, manifest) {
  const toolsWithTiers = (spec.tools ?? []).map((tool) => ({
    ...tool,
...
function buildToolContracts(spec, manifest) {
...
function buildContractFiles(spec, manifest) {
  const builders = {
    "contracts/spec-profile.json": () => `${JSON.stringify(buildSpecProfileContract(spec, manifest), null, 2)}\n`,
    "contracts/system-boundary.yaml": () => buildSystemBoundaryContract(spec, manifest),
    "contracts/tool-contracts.yaml": () => buildToolContracts(spec, manifest),
...
export function buildAgentArtifacts(input, options = {}) {
  ...
  const contractFiles = buildContractFiles(spec, manifest);
  const packageFiles = [...new Set([...CORE_FILES, ...contractFiles.map((file) => file.path)])];
  const agentConfig = {
